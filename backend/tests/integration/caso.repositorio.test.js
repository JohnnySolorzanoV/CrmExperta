...
describe('caso.repositorio', () => {
  ...
  it('INT-REPO-CASOS-01 obtenerTodos transforma filas en entidades Caso con datos consistentes', async () => {
    var r = await repo.obtenerTodos()
    expect(r.length).toBeGreaterThan(0)
    expect(r[0].nombreCaso).toBeTruthy()
  })

  it('INT-REPO-CASOS-02 buscarPorId retorna null cuando el caso no existe', async () => {
    var r = await repo.buscarPorId(999)
    expect(r).toBeNull()
  })

  it('INT-REPO-CASOS-03 actualizarEstado retorna la entidad actualizada cuando existe coincidencia', async () => {
    var r = await repo.actualizarEstado(casoId, 'cerrado')
    expect(r?.estadoCaso).toBe('cerrado')
  })

  it('INT-REPO-CASOS-04 actualizarNotasConclusiones persiste y retorna notas y conclusiones actualizadas', async () => {
